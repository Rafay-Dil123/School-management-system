...
function Teachermarks() {
  ...
  const getInitialData = async () => {
    try {
      const response = await fetch(`/api/studentsdata/${selectedSection}/${selectedTest}`);
...
const handleSaveMarksToBackend = async () => {
  try {
    ...
    for (const student of studentData) {
      const response = await fetch(`/api/update_student_marks/${selectedSection}/${selectedTest}/${student.rollno}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
